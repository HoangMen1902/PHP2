...
class ProductController extends BaseController
{
    ...
    public function deleteProperty()
    {
        header('Content-Type: application/json');
        $OptionValueModel = new OptionValueModel();
        $result = $OptionValueModel->deleteValue($_POST['id']);
    ...
    public function deleteSku()
    {
        header('Content-Type: application/json');
        $SkuModel = new SkuModel();
        $result = $SkuModel->deleteSku($_POST['id']);
    ...
    public function update($id)
    {
        ...
        $productUpdate = $ProductModel->updateProduct($id, $productInfo);
        if (!$productUpdate) {
            Notification::error('Lỗi', 'Sửa sản phẩm thất bại');
            header('location: /admin/product/detail/' . $id);
            exit();
        }

        $firstSkuId = $_POST['sku_id'];


        $firstSkuUpdate = $SkuModel->updateSku($firstSkuId, $firstSku);
        if (!$firstSkuUpdate) {
            Notification::error('Lỗi', 'Sửa SKU đầu tiên thất bại');
            header('location: /admin/product/detail/' . $id);
            exit();
        }
        ...
        $firstOptionValues = explode(',', $data[0]['value_ids']);
        $firstSkuVal = explode(',', $data[0]['sku_value_ids']);



        foreach ($_POST['options_base'] as $index => $option) {
            $firstOption = [];
            $firstOption['option_id'] = trim($option);
            $firstOption['value_name'] = trim($_POST['values_base'][$index]);
            $skuValOption['option_id'] = trim($option);

            $firstOptionUpdate = $OptionValueModel->updateValue($firstOptionValues[$index], $firstOption);
            $skuOptionValUpdate = $SkuValueModel->updateSkuValue($firstSkuVal[$index], $skuValOption);
            if (!$firstOptionUpdate || !$skuOptionValUpdate) {
                Notification::error('Lỗi', 'Cập nhật thông tin thuộc tính đầu tiên thất bại');
                header('location: /admin/product/detail/' . $id);
                exit();
            }
        }
